...
## Round 12 update (Flagged entities row-gap bug fixed)

Team feedback: "the flagged entities are overlapping, no gap between the top and bottom row." Root cause confirmed precisely: the entity-pill list is a wrapping auto-layout row with a 6px gap *between pills in the same row*, but the gap *between wrapped rows* was set to 0 — so when a pill wrapped to a second line, it sat flush against the row above with no vertical breathing room. Fixed on all three instances that use this compact context-rail layout (Review Workspace, and its two clones on Session-expired-reauth and Connection-lost) by setting the row gap to match the pill gap (6px). Each container auto-grew and everything below it reflowed automatically — no manual repositioning needed. AI Analysis Summary's own Flagged entities section uses a wider, single-row layout that never wraps, so it wasn't affected and wasn't touched.

## Round 13 update (sign-in/auth requirements closed, InlineNotification contrast fix)

Prompted by a cross-file audit while building the companion Manager prototype: `Requirements-BA-FINAL.md` was re-checked against every screen in this file for `AR-AS-06` and the newer `SR-SA-01`–`05` section (item 5 in Assumptions & Open Decisions below). Two real gaps found and fixed:

- **`SR-SA-05` (secure sign-out) was entirely unbuilt.** No screen had any sign-out affordance at all. Added a "Sign out" link to the `Header=Auditor` master component (`02 — Components`) — propagated automatically to 17 of 18 header-bearing screens; the 18th (Exposure Limit Reached, whose header was detached from the master during the Round 9 exposure-bar fix) was fixed individually since a detached instance no longer receives master-level updates. Wired on all 18 screens to navigate back to Login, ending the session — satisfying `SR-SA-05`'s "returned to the Staff Portal login screen" requirement.
- **`SR-SA-01` (dedicated staff URL) is now resolved in Jana's doc** (previously "to be confirmed," now a concrete subdomain/path approach, organisation-provisioned) — this is an infra/routing decision, not a Figma screen, so it's documented on the Login Requirements Panel rather than built as UI. `SR-SA-02`/`03`/`04` remain implicit in the existing Login/role-scoped design, same as previously assessed.
- **Found a real WCAG 2.2 AA contrast failure while auditing the shared `InlineNotification` component** (same audit pass, not initially in scope but worth fixing at the source since the Manager file inherited this component by direct reuse): the Success and Error tones used their accent colour as literal prefix text on a pale tint background — measured by precise luminance calculation at ~3.0:1 (Success) and ~4.28:1 (Error), both below the 4.5:1 minimum for normal-size text. Fixed at the master component and both live instances (Login — Error, Login — Locked-out) to use Gray 100 prefix text uniformly, matching the Warning tone's already-correct pattern — tone is still conveyed via the tinted background and left-accent bar, consistent with this file's own "colour is never the sole signal" rule.
...
## Round 14 update (team feedback assessed on its merits, one real gap closed)

Team feedback: "make sure session expiry / connection loss does not reset blur, mute, or review progress" and "keep cooldown, exposure limit, and case status clear with text, not colour only." Assessed each on evidence before touching anything, per the brief to only act on feedback backed by sound reasoning.
