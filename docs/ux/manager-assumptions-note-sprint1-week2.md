...
| 3 | Brief note on Manager's likely background/context | [Section 3 — Manager's Likely Background/Context](#3-managers-likely-backgroundcontext-brief-note) — also keeps the Auditor's context alongside it, since the manager flow exists to support that person specifically |
| 4 | Master documents (requirements, architecture, or persona files, as applicable) updated to reflect this deliverable | [Section 4 — Master Document Updates](#4-master-document-updates) — lists every persona file and doc that now cross-references this note |
